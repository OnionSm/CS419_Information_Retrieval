...
def connect_to_mongodb():
    ...
    # print(MONGODB_USERNAME, MONGODB_PASSWORD, MONGODB_HOST, MONGODB_PORT, MONGODB_DATABASE, AUTH_MECHANISM, AUTH_SOURCE)
    global client, db
    ...
    #     return None, None
        ...
        error_msg = f"Lỗi kết nối MongoDB: Không thể kết nối tới server tại {MONGODB_HOST}:{MONGODB_PORT}. Chi tiết: {e}"
        print(error_msg)
        raise ConnectionError(error_msg) from e

    except OperationFailure as e:
        error_msg = f"Lỗi xác thực MongoDB: Vui lòng kiểm tra tên người dùng, mật khẩu và auth_source ('{AUTH_SOURCE}'). Chi tiết: {e}"
        print(error_msg)
        raise AuthenticationError(error_msg) from e

    except ValueError as e:
        error_msg = f"Lỗi: PORT '{os.environ.get('MONGODB_PORT')}' không phải là số nguyên hợp lệ."
        print(error_msg)
        raise ValueError(error_msg) from e

    except Exception as e:
        error_msg = f"Một lỗi không xác định đã xảy ra trong quá trình kết nối MongoDB: {e}"
        print(error_msg)
